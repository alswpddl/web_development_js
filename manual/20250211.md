...
일반적으로 변수는 camel case 표기를 따르지만, 상수의 겨우 대문자로 이루어진 snake case를 사용합니다.
...
### switch문으로의 개선
: 일반적으로는 if문이 더 많이 쓰입니다. 하지만 하나의 변수를 어려 값과 비교하는 경우에 switch문으로 교체하도록 합니다. 그렇기 때문에 전체적인 가독성을 고려하여 선택하는 편을 추천합니다.
...
#### 이벤트 핸들러를 onclick에 모두 작성했을 때의 비효율성을 확인할 결과 개발자들에게 필요한 것은? 
...
예를 들어 숫자 두 개를 argument로 제공했을 때 우리가 필요한 결과값은 덧셈/뺄셈/곱셈/나눗셈/제곱 등등이 필요할 뿐이지 함수 내부에 정의되어 있는 방식,
...
- 함수의 세부 구현 방식을 추상화시켜 프로그래밍 내 다른 부분과의 영향도를 최소로 줄일 수 있다는 점이 바로 함수의 장점에 해당합니다.
...
하지마 순서대로 argument가 매개 변수에 대입되기 떄문에 (1,2,3)의 경우 3은 신경쓰지 않고 2까지만 대입이 되고 연산은 정상적으로 이루어지는 것처럼 보입니다.
...
function onScissorsClick() {    // 버튼 클릭했어요 -> userInput prompt가 필요 없죠
  ...
  else if (randomNum >= 0.33 && randomNum < 0.66) comInput = ROCK;
  else comInput = PAPER;
